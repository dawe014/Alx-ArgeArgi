...
document.addEventListener("DOMContentLoaded", () => {
  const form = document.querySelector(".update_rent_home");
  ...
  button.addEventListener("click", async (event) => {
    ...
    if (file) {
      ...
      const res = await axios
        .post(`/api/v1/renthouse/${homeId}/addimage`, formData, {
          headers: {
  ...
  if (form) {
    form.addEventListener("submit", (e) => {
      e.preventDefault();
      console.log('Dawe from rent house')
...
async function deleteImage(homeId, imageUrl, index) {
  try {
    ...
    await axios.delete(`/api/v1/renthouse/${homeId}/image`, {
      data: {
        imageUrl: imageUrl,
      },
    });
